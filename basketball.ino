...
void setup()
{
  ...
  Wire.begin(); // I2C setup, we don't need to specify address for Master
...
void loop()
{
  ...
  if (flag == 1 && game_start == 0)
  {
    ...
    game_start = 1;
    servo_up();                         // open the gate
    display_time_score(re_time, score); // reset time & score to zero
    time_previous = millis();           // record the current time (for countdown)
  }
  ...
  if (game_start == 1)
  {
    ...
    Serial.println(a0);
    if (a0 < 600)
    { // check whether the ball passed
      delay(100);
      score += 1;
      display_time_score(re_time, score);

      Wire.beginTransmission(3); // Start the trannsmmission, target address is 3
      Wire.write("hello \n");    // write in hello \n
